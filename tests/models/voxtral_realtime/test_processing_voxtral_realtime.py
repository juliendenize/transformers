...
class VoxtralRealtimeProcessorTest(unittest.TestCase):
    ...
    def _assert_no_mistral_common_deprecation(self, caught):
        offending = [
            str(w.message)
            for w in caught
            if issubclass(w.category, (DeprecationWarning, FutureWarning)) and "mistral_common" in (w.filename or "")
        ]
        self.assertEqual(offending, [], f"Unexpected mistral-common deprecation warning(s): {offending}")
    ...
    def test_offline_call(self):
        ...
        self._assert_no_mistral_common_deprecation(caught)
    ...
    def test_online_streaming_matches_low_level_audio_encoder(self):
        # Online streaming builds the prefill from the audio encoder primitives instead of the deprecated
        # audio-carrying TranscriptionRequest, and must not emit the FutureWarning that path still does.
        processor = self.processor
        sampling_rate = processor.feature_extractor.sampling_rate
        audio = self._dummy_audio(seed=42)

        # Our path: no audio is passed to a request, so no mistral-common deprecation/future warning.
        with warnings.catch_warnings(record=True) as caught:
            warnings.simplefilter("always")
            encoding = processor(audio, is_streaming=True, is_first_audio_chunk=True, return_tensors="pt")
        self._assert_no_mistral_common_deprecation(caught)

        # The prompt and audio prefill are reconstructed from the same low-level audio encoder primitives.
        ...
        # Document why we migrated: the deprecated audio-in-request path still emits a streaming FutureWarning.
        ...
        with warnings.catch_warnings(record=True) as legacy_caught:
            warnings.simplefilter("always")
            try:
                processor.tokenizer.tokenizer.encode_transcription(
        ...
        legacy_streaming_warnings = [
            w for w in legacy_caught if issubclass(w.category, FutureWarning) and "streaming" in str(w.message).lower()
        ]
        self.assertTrue(
            legacy_streaming_warnings,
            "Expected the legacy audio-in-request path to emit a streaming FutureWarning.",
        )
