        ...
        **kwargs: Unpack[VoxtralRealtimeProcessorKwargs],
        ...
        if is_first_audio_chunk:
            ...
            for audio_el in audio:
                if is_streaming:
                    # In online streaming mode, build the prefill prompt and the left padding from the
                    # low-level audio encoder primitives rather than passing audio to a (deprecated)
                    # audio-carrying `TranscriptionRequest`. The prompt is a fixed streaming prefix and the
                    # actual audio is carried by the extracted features (prepended with the left padding).
