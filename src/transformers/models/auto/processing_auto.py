    ...
    def from_pretrained(cls, pretrained_model_name_or_path, **kwargs):
        ...
        if processor_class is None:
            ...
            try:
                ...
                # Config loading failed — probe for native Mistral checkpoints
                # (no config.json, only params.json + tekken.json).
                # Only fall through for missing-file / unrecognized-model errors;
                # re-raise genuine network or permission errors.
                _should_probe = isinstance(config_error, ValueError)
                if isinstance(config_error, OSError):
                    _err_msg = str(config_error).lower()
                    _should_probe = (
                        "does not appear to have" in _err_msg
                        or "is not a local folder" in _err_msg
                        or "can't find" in _err_msg
                    )
                if _should_probe:
